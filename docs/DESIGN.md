...
**Print's label is just "พิมพ์" (fixed 2026-06-30)** — it was "พิมพ์ / Export PDF", which read as two bundled actions when `handleExport` → `exportPDF()` is a single action (popup window + `window.print()`, see §12). There are exactly 3 possible items, never more: Edit, Print, Delete — don't add a separate "Export PDF" entry, it's the same action as Print.
