...
**Rejected rows get a standalone button, not a kebab item.** A rejected request's "edit" is really "resubmit" — same form, different context — so it's surfaced as its own `<Button variant="primary">` (icon `RefreshIcon`, label "ส่งใหม่") sitting directly in the row, to the left of the kebab. **Primary, not secondary** — secondary's plain white fill was too quiet to read as "this row needs action" next to otherwise-identical rows; the solid gradient is the one deliberately loud button in the whole table. The kebab on a rejected row drops "Edit" (it's now the standalone button) but keeps "Print". This is why the action column widened `10% → 16%` (taken from "ลูกค้า" 22%→18% and "อัปเดต" 13%→11%, the two columns with the most slack) — it now has to fit a labeled button plus the kebab side by side, not just one icon button.
